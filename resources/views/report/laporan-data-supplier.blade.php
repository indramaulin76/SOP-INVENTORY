@extends('layouts.app')
@section('title', 'Laporan Data Supplier')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="filter-container">
                ...
                <form method="GET" action="{{ route('report.laporan-data-supplier') }}">
                    <div class="filter-row">
                        <div class="filter-group">
                            <label>Cari (Nama/Kode Supplier)</label>
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau kode supplier...">
                        </div>
                    </div>
                    ...
                    <div class="filter-actions">
                        ...
                        <a href="{{ route('report.laporan-data-supplier') }}" class="btn-export btn-export-pdf" style="text-decoration: none; display: inline-block;">
                            🔄 Reset
                        </a>
        ...
        <div class="card-body">
            <div class="table-container">
                            ...
                            <tr>
                                ...
                                @if(auth()->check() && in_array(auth()->user()->role, ['admin', 'superadmin']))
                                    <td class="action-cell">
                                        <form action="{{ route('input-data.destroy-supplier', $supplier->id) }}" method="POST" style="display: inline;" onsubmit="return confirmDelete('{{ $supplier->nama_supplier }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn-delete" title="Hapus data supplier">
                                                🗑️ Hapus
                                            </button>
                                        </form>
                                    </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ auth()->check() && in_array(auth()->user()->role, ['admin', 'superadmin']) ? '9' : '8' }}" style="text-align: center; color: #999; padding: 40px;">
                                    Tidak ada data supplier tersedia
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <div class="table-footer">
                    <span class="total-label">Total Supplier: <strong>{{ $data->count() }}</strong></span>
                </div>
            </div>
        </div>
    </div>
...
@section('scripts')
    <script>
        ...
        function confirmDelete(namaSupplier) {
            return confirm('Apakah Anda yakin ingin menghapus data supplier "' + namaSupplier + '"?\n\nTindakan ini tidak dapat dibatalkan!');
        }
    </script>
@endsection
