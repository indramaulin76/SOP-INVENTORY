...
@section('title', 'Barang Dalam Proses')

@section('content')
<div class="card">
    <div class="card-header">
        <h2>Form Input Barang Dalam Proses</h2>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        <form action="{{ route('input-barang.store-proses') }}" method="POST" id="mainForm">
            @csrf
            ...
            <div class="form-actions" style="margin-top: 20px;">
                ...
                <button type="submit" class="btn btn-primary">Simpan Pemasukan Barang 💾</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // Hanya untuk barang dengan kategori "Barang Dalam Proses"
    async function loadBarangByName(input) {
        ...
        try {
            ...
            if (data.success && data.barang && Array.isArray(data.barang)) {
                ...
                if (!isMatch && kategori !== '') {
                    alert('Barang "' + selectedItem.nama_barang + '" bukan kategori Barang Dalam Proses. Hanya barang dengan kategori "Barang Dalam Proses" yang dapat digunakan untuk input barang dalam proses.');
                    namaInput.value = '';
                    if (kodeInput) kodeInput.value = '';
                    if (hargaInput) hargaInput.value = '0';
                    if (qtyInput) calculateRow(qtyInput);
