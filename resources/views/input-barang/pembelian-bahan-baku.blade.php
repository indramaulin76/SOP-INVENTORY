...
<div class="card">
    <div class="card-header">
        <h2>Form Input Pembelian Bahan Baku</h2>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        <form action="{{ route('input-barang.store-pembelian') }}" method="POST" id="mainForm">
            @csrf
            ...
            <div class="form-row">
                <div class="form-group" style="position: relative;">
                    ...
                    <input type="text" id="nama_supplier" name="nama_supplier" class="input autocomplete-input" value="{{ old('nama_supplier') }}" placeholder="Nama Supplier" autocomplete="off">
                    <div id="supplier-autocomplete" class="autocomplete-dropdown"></div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="kode_supplier" name="kode_supplier" class="input" value="{{ old('kode_supplier') }}" placeholder="SUP001" readonly style="background-color: #f5f5f5;" required>
...
<script>
    ...
    // ============================================
    // AUTOCOMPLETE LOGIC FOR SUPPLIER
    // ============================================
    ...
    function initSupplierAutocomplete() {
        const input = document.getElementById('nama_supplier');
        const dropdown = document.getElementById('supplier-autocomplete');
        ...
        const searchSupplier = debounce(async (term) => {
            if (term.length < 2) {
            ...
            try {
                const response = await fetch(`/api/supplier/search?term=${encodeURIComponent(term)}`);
                const results = await response.json();
                currentResults = results;
                selectedIndex = -1;
                
                if (results.length === 0) {
                    dropdown.innerHTML = '<div class="autocomplete-item">Tidak ada supplier ditemukan</div>';
                    dropdown.classList.add('show');
                    return;
                }
                
                dropdown.innerHTML = results.map((supplier, index) => `
                    <div class="autocomplete-item" data-index="${index}">
                        <strong>${supplier.nama_supplier}</strong>
                        <small>Kode: ${supplier.kode_supplier}</small>
                    </div>
                `).join('');
        ...
        function selectSupplier(supplier) {
            input.value = supplier.nama_supplier;
            kodeInput.value = supplier.kode_supplier;
            dropdown.classList.remove('show');
            currentResults = [];
        }
        ...
        input.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowDown') {
                ...
                if (currentResults[selectedIndex]) {
                    selectSupplier(currentResults[selectedIndex]);
                }
