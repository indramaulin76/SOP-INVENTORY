    ...
    async function loadBarangByName(input) {
        ...
        try {
            ...
            if (data.success && data.barang && Array.isArray(data.barang)) {
                ...
                if (!isMatch && kategori !== '') {
                    alert('Barang "' + selectedItem.nama_barang + '" bukan kategori Bahan Baku. Hanya barang dengan kategori "Bahan Baku" yang dapat digunakan untuk pemakaian bahan baku.');
                    namaInput.value = '';
                    if (kodeInput) kodeInput.value = '';
                    if (hargaInput) hargaInput.value = '0';
                    if (qtyInput) calculateRow(qtyInput);
